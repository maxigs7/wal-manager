...
ALTER FUNCTION public.get_balance(uuid, timestamp without time zone)
    OWNER TO postgres;

GRANT EXECUTE ON FUNCTION public.get_balance(uuid, timestamp without time zone) TO PUBLIC;

GRANT EXECUTE ON FUNCTION public.get_balance(uuid, timestamp without time zone) TO anon;

GRANT EXECUTE ON FUNCTION public.get_balance(uuid, timestamp without time zone) TO authenticated;

GRANT EXECUTE ON FUNCTION public.get_balance(uuid, timestamp without time zone) TO postgres;

GRANT EXECUTE ON FUNCTION public.get_balance(uuid, timestamp without time zone) TO service_role;

DROP FUNCTION IF EXISTS public.get_balance("endDate" timestamp without time zone);
...
AS $BODY$
  ...
  select
    ...
    null as "subCategory",
    null as type
  from public.get_balance("accId", "startDate" - interval '1 seconds')
  where amount > 0
