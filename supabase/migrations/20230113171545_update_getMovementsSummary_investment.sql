...
AS $function$
  select
    ...
    sum(case when m.month = "pPreviousMonth" and m.year = "pPreviousYear" and m.type = 'investment' and m.amount < 0 then abs(m.amount) else 0 end) as "previousInvestment",
    ...
    sum(case when m.month = "pCurrentMonth" and m.year = "pCurrentYear" and m.type = 'investment' and m.amount < 0 then abs(m.amount) else 0 end) as "currentInvestment"
